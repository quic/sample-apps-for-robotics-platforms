...
if [ "$flag" = 'on' ]; then
    ...
    if [ -z "$judge_conf" ]; then
        ...
        wpa_supplicant -i$network_card -Dnl80211 -c /data/misc/wifi/wpa_supplicant.conf -O /data/misc/wifi/sockets -B
    else
    ...
    if [ "$judge_result" = "" ]; then
        ...
        ret_select=$(wpa_cli -i wlan0 select_network $num -p /data/misc/wifi/sockets)
        echo -e "\e[32mSelect WIFI $ret_select.\e[0m"

        # enable SSID #
        ret_enable=$(wpa_cli -i $network_card enable_network $num -p /data/misc/wifi/sockets)
