...
int main(int argc, char *argv[]) {
    ...
    GstElement *qtimlesnpe, *queue, *qtioverlay;
    GstBus *bus;
    GstMessage *msg;
    ...
    qtimlesnpe      =   gst_element_factory_make ("qtimlesnpe", "qti-mlesnpe");
    ...
    if (!pipeline || !source || !sink || !capsfilter || !qtimlesnpe || !queue || !qtioverlay) {
        g_printerr ("Not all elements could be created.\n");
        return -1;
    }

    /* Build the pipeline */
    gst_bin_add_many (GST_BIN (pipeline), source, capsfilter, qtimlesnpe, queue, qtioverlay, sink, NULL);
    if (gst_element_link_many (source, capsfilter, qtimlesnpe, queue, qtioverlay, sink, NULL) != TRUE) {
    ...
    g_object_set (qtimlesnpe, "config", "/data/misc/camera/snpe_classification.config", 
         "postprocessing", "classification", NULL);
