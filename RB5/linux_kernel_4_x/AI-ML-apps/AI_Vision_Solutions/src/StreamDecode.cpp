...
void StreamDecode::SetRtspUrl(string url)
{
    this->rtspUrl_ = url;
}

StreamDecode::StreamDecode(std::string streamtype, std::string url, int camera_fps)
{
    this->rtspUrl_ = url;
...
int StreamDecode::gst_camera_pipeline_init() 
{
    ...
    filtercaps = gst_caps_new_simple ("video/x-raw",
        ...
        "framerate", GST_TYPE_FRACTION, this->camera_fps, 1,
        NULL);
...
int StreamDecode::gst_rtsp_pipeline_init()
{
    ...
    /* Create the elements */
    // rtspsrc! h264depay ! h264parse ! qtivdec ! qtivtransform ! video/x-raw,format=BGR ! appsink
    data_.source.reset(gst_element_factory_make("rtspsrc", "source"), UnRefElement);
    data_.videoDepay.reset(gst_element_factory_make("rtph264depay", "h264depay"), UnRefElement);
    data_.videoParse.reset(gst_element_factory_make("h264parse", "h264parse"), UnRefElement);
    data_.h264dec.reset(gst_element_factory_make("qtivdec", "avdec_h264"), UnRefElement);
    data_.transform.reset(gst_element_factory_make("qtivtransform", "transform"), UnRefElement);
    data_.sink.reset(gst_element_factory_make("appsink", "sink"), UnRefElement);
         
    if (!data_.pipeline.get() || !data_.source.get() || !data_.videoDepay.get() || !data_.videoParse.get() || !data_.h264dec.get() || !data_.transform.get() || !data_.sink.get())
    {
        LOG_ERROR("Not all elements could be created.");
        return QS_ERROR;
    }

    gst_bin_add_many(GST_BIN(data_.pipeline.get()), data_.source.get(), data_.videoDepay.get(), data_.videoParse.get(), data_.h264dec.get(), data_.transform.get(), data_.sink.get(), NULL);
    if (!gst_element_link_many(data_.videoDepay.get(), data_.videoParse.get(), data_.h264dec.get(), data_.transform.get(), data_.sink.get(), NULL))
    {
        LOG_ERROR("Elements could not be linked.\n");
        // gst_object_unref(data.pipeline);
        return QS_ERROR;
    ...
    /* Connect to the pad-added signal */
    g_signal_connect(data_.source.get(), "pad-added", G_CALLBACK(StreamDecode::OnPadAdd), data_.videoDepay.get());
    return QS_SUCCESS;
}
...
{
    shared_ptr<StreamDecode> decodePtr = make_shared<StreamDecode>(input_config->StreamType, input_config->Url, input_config->camera_fps);
