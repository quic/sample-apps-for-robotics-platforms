    ...
    if (solutions.isArray()) {
        int size = solutions.size();
            for (int i = 0; i < size; ++i) {
                ...
                if (solutions[i][model_name].isArray()) {
                    int num = solutions[i][model_name].size();
                    for (int j = 0; j < num; j++) {
                        solutionconfig->model_name.push_back( solutions[i][model_name][j].asString()  );
                        //std::cout<<"Model name = "<< solutions[i][model_name][j].asString()<<std::endl;
                    }
                }
