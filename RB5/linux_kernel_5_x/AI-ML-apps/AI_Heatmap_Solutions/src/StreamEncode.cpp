...
/** @brief To initialize filesink pipeline
 * @param output_path stores the output file at this location
*/
int StreamEncode::gst_filesink_pipeline_init(string output_type, string output_path) 
{
    ...
    data.x264enc.reset(gst_element_factory_make("qtic2venc", "qtic2venc"), DefaultUnRefGstElement);
    data.h264parse.reset(gst_element_factory_make("h264parse", "h264parse"), DefaultUnRefGstElement);
    ...
    if (!data.pipeline.get() || !data.appsrc.get() || !data.vidconv.get() || !data.vtransform.get() || !data.capsfilter.get() ||
        !data.x264enc.get() || !data.h264parse.get() || !data.qtmux.get() || !data.filesink.get())
    {
        LOG_ERROR("[not all element created,(%s)(%s)(%s)(%s)(%s)(%s)(%s)(%s)(%s)]\n",
            ...
            !data.x264enc.get() ? "ng" : "ok",
            !data.h264parse.get() ? "ng" : "ok",
    ...
    g_object_set(G_OBJECT(data.x264enc.get()), "control-rate", "max-bitrate", NULL);
    g_object_set(G_OBJECT(data.x264enc.get()), "target-bitrate", 6000000, NULL);
    g_object_set(G_OBJECT(data.x264enc.get()), "interval-intraframes", 29, NULL);
    g_object_set(G_OBJECT(data.x264enc.get()), "periodicity-idr", 1, NULL);
    ...
    if (!gst_element_link(data.capsfilter.get(), data.x264enc.get()))
    {
        LOG_ERROR("Link Fail %s %s \n", GST_ELEMENT_NAME(data.capsfilter.get()), GST_ELEMENT_NAME(data.x264enc.get()));
        return QS_ERROR;
    }

    if (!gst_element_link(data.x264enc.get(), data.h264parse.get()))
    {
        LOG_ERROR("Link Fail %s %s \n", GST_ELEMENT_NAME(data.x264enc.get()), GST_ELEMENT_NAME(data.h264parse.get()));
        return QS_ERROR;
    }

    if (!gst_element_link(data.h264parse.get(), data.qtmux.get()))
    {
        LOG_ERROR("Link Fail %s %s \n", GST_ELEMENT_NAME(data.h264parse.get()), GST_ELEMENT_NAME(data.qtmux.get()));
        return QS_ERROR;
    }
...
/** @brief To set wayland pipeline for display
*/
...
/** @brief To check whether wayland or filesink 
 * @param output_type wayland or filesink
 * @param output_path only for filesink to store the file at this location
