...
int StreamDecode::gst_rtsp_pipeline_init()
{
    ...
    data_.videoDepay.reset(gst_element_factory_make("rtph264depay", "rtph264depay"), UnRefElement);
    ...
    if (!data_.pipeline.get() || !data_.source.get() || !data_.videoDepay.get() || !data_.videoParse.get() ||  !data_.transform.get() || !data_.sink.get())
    {
        LOG_ERROR("Not all elements could be created.");
        return QS_ERROR;
    }

    gst_bin_add_many(GST_BIN(data_.pipeline.get()), data_.source.get(), data_.videoDepay.get(), data_.videoParse.get(), data_.transform.get(), data_.sink.get(), NULL);
    if (!gst_element_link_many(data_.videoDepay.get(), data_.videoParse.get(), data_.transform.get(), data_.sink.get(), NULL))
    {
        LOG_ERROR("Elements could not be linked.\n");
        // gst_object_unref(data.pipeline);
        return QS_ERROR;
