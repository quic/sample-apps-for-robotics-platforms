...
int gst_encode_yuv () {
    ...
    GstElement *pipeline, *source, *videoparse, *qtic2venc, *h264parse, *sink, *capsfilter, *mp4mux;
    GstCaps *caps;
    ...
    if (!pipeline || !source || !videoparse || !qtic2venc || !h264parse || !mp4mux || !sink) {
    ...
    /* Build the pipeline */
    gst_bin_add_many(GST_BIN(pipeline), source, videoparse, qtic2venc,
            h264parse, mp4mux, sink, NULL);
    if (gst_element_link_many(source, videoparse, qtic2venc, h264parse, mp4mux, sink, NULL) != TRUE ) {
...
int gst_decode_file () {
   ...
   if (gst_element_link(source, qtdemux) != TRUE ) {
    ...
    if (gst_element_link_many(queue, h264parse, qtic2vdec, sink, NULL) != TRUE ) {
        g_printerr("queue, h264parse, qtic2vdec, and sink could not be linked\n");
        gst_object_unref(pipeline);
        return GST_STATE_NULL;
    }
