...
static int gethdmiinfo(char* hdmi_info)
{
    char hdmi_info_file[64] = {0};
    FILE* fp;

    snprintf(hdmi_info_file, sizeof(hdmi_info_file), 
            "/sys/class/drm/card0-DSI-1/modes");

    fp = fopen(hdmi_info_file, "r");
    if (fp == NULL) {
        printf("Can not open file: %s\n", hdmi_info_file);
...
int main()
{
    ...
    ret = gethdmiinfo(hdmi_info);
    if (ret == 0)
        printf("HDMI information      : %s\n", hdmi_info);
    else
        printf("HDMI information get error!\n");
