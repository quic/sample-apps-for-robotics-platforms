...
class CamThermalDriver(IDriver):
	"""
	Driver implementation for an Thermal Camera logger
	"""
	def __init__(self, log_path, rate):
		super(CamThermalDriver, self).__init__(log_path, DriverTypes.CamThermal, rate)
	...
	def start(self, duration):
		...
		try:
			...
			print("ERROR unable to create logging directory for cam thermal")
			return False
		...
		if self.simulation:
			...
			while not super(CamThermalDriver,self).duration_ended(start_time, duration):
