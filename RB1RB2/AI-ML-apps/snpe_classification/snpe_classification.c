...
int main(int argc, char *argv[]) {
...
#ifdef PARSER
    /* Create the pipeline */
    pipeline = gst_parse_launch("qtiqmmfsrc name=camsrc ! video/x-raw,format=NV12,width=1280,height=720,framerate=30/1 ! \
                                 queue ! tee name=split ! queue ! qtimetamux name=metamux ! queue ! qtioverlay ! queue ! \
				 waylandsink sync=false fullscreen=true enable-last-sample=false  split. ! queue ! \
				 qtimlvconverter ! queue ! qtimlsnpe delegate=none model=/data/mobilenet_v1_quantaware_quantized.dlc ! \
				 queue ! qtimlvclassification threshold=20.0 results=3 module=mobilenet labels=/data/mobilenet2.labels ! \
	...
	!qtimlvconverter || !qtimlsnpe || !qtimlvclassification ||
    ...
    if (!gst_element_link_many (source, capsfilter, tee, NULL) ||
        ...
        !gst_element_link_many (infqueue, qtimlvconverter, qtimlsnpe, qtimlvclassification, NULL) ||
    ...
    g_object_set (qtimlsnpe, "model", "/data/mobilenet_v1_quantaware_quantized.dlc", "delegate", 1, NULL);
    g_object_set (qtimlvclassification, "threshold", 20.0, "results",3, "module",1,
		  "labels", "/data/mobilenet2.labels", NULL);
