...
static int gst_sample_playback(const char* content)
{
    ...
    alsasink = gst_element_factory_make("autoaudiosink", "alsa_sink");
    pipeline = gst_pipeline_new("audio_playback_stream");

    if (!pipeline)
        g_printerr("%s: ---yaozhao---!pipline\n", __func__);
    if (!wavparse)
        g_printerr("%s: ---yaozhao---!wavparse\n", __func__);
    if (!alsasink)
        g_printerr("%s: ---yaozhao---!alsasink\n", __func__);
    if (!filesrc)
        g_printerr("%s: ---yaozhao---!filesrc\n", __func__);
...
static int gst_sample_capture(const char* content)
{
    ...
    /* create pipeline */
    alsasrc = gst_element_factory_make("pulsesrc", "alsa_src");
    ...
    if (!pipeline)
        g_printerr("%s: ---yaozhao---!pipline\n", __func__);
    if (!capsfilter)
        g_printerr("%s: ---yaozhao---!capsfilter\n", __func__);
    if (!wavenc)
        g_printerr("%s: ---yaozhao---!wavenc\n", __func__);
    if (!filesink)
        g_printerr("%s: ---yaozhao---!filesink\n", __func__);
    if (!alsasrc)
        g_printerr("%s: ---yaozhao---!alsasrc\n", __func__);
    if (!pipeline || !alsasrc || !capsfilter || !wavenc || !filesink) {
        g_printerr("%s: Not all elements could be created\n", __func__);
        return GST_STATE_NULL;
    }

    g_object_set(G_OBJECT(alsasrc), "num-buffers", 500, NULL);
