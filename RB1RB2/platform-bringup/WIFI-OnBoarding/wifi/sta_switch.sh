...
judge_conf=$(cat /etc/wlan/wpa_supplicant.conf | grep update_config=1)
if [ -z "$judge_conf" ]; then
    sed '3iupdate_config=1' /etc/wlan/wpa_supplicant.conf -i
...
if [ "$flag" = 'on' ]; then
    ...
    if [ -z "$judge_conf" ]; then
        ...
        wpa_supplicant -i$network_card -Dnl80211 -c /etc/wlan/wpa_supplicant.conf -O /etc/wlan/sockets -B
    else
