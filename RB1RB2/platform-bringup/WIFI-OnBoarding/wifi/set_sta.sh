...
wpa_supplicant -iwlan0 -Dnl80211 -c /etc/wlan/wpa_supplicant.conf -O /etc/wlan/sockets -B

# Add network #
ret_NetID=$(wpa_cli -i $network_card add_network -p /etc/wlan/sockets)
...
ret_WifiID=$(wpa_cli -i $network_card set_network $ret_NetID ssid  '"'$new_ssid'"' -p /etc/wlan/sockets)
if [ "$ret_WifiID" = 'OK' ]; then
    ...
    ret_WifiPSK=$(wpa_cli -i $network_card set_network $ret_NetID psk  '"'$new_psk'"' -p /etc/wlan/sockets)
    ...
    if [ "$ret_WifiPSK" = 'OK' ]; then
        ...
        judge_ssid=$(cat /etc/wlan/wpa_supplicant.conf | grep -o ssid=\"$new_ssid\")
        judge_psk=$(cat /etc/wlan/wpa_supplicant.conf | grep -A10 $new_ssid | grep -m 1 psk= | grep -o psk=\"$new_psk\")
        ...
        if [ "$judge_ssid" == "ssid=\"$new_ssid\"" ] && [ "$judge_psk" == "psk=\"$new_psk\"" ]; then
            ...
            ret_rm=$(wpa_cli -i $network_card remove_network $ret_NetID -p /etc/wlan/sockets)
            ...
            ret_list=$(wpa_cli -i $network_card list_network -p /etc/wlan/sockets)
            judge_list=`echo "$ret_list" |  sed -n "/$new_ssid/p" | awk '{print $1}'`
            # Enable saved WiFi #
            ret_enable=$(wpa_cli -i $network_card enable_network $judge_list -p /etc/wlan/sockets)
            echo -e "\e[32mEnable #$judge_list WIFI as you think $ret_enable!\e[0m"
        else
            # Enabling network #
            ret_enable_network=$(wpa_cli -i $network_card enable_network "$ret_NetID"  -p /etc/wlan/sockets)
            ...
            ret_dave_config=$(wpa_cli -i  $network_card save_config -p /etc/wlan/sockets)
        ...
        ret_remove_network=$(wpa_cli -i $network_card remove_network "$ret_NetID!" -p /etc/wlan/sockets)
