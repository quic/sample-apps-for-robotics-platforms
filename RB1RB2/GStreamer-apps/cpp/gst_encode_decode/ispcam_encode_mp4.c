...
{
    GstElement *pipeline, *source, *framefilter, *parse, *muxer, *queue, *sink, *qtic2venc;
    int res;
    gint8 camera_id;
    ...
    framefilter = gst_element_factory_make("capsfilter",   "capsfilter0");
    parse       = gst_element_factory_make("h264parse",    "h264parse0");
    qtic2venc   = gst_element_factory_make("qtic2venc",    "c2_vencencoder0");
    muxer       = gst_element_factory_make("mp4mux",       "mp4mux0");
    queue       = gst_element_factory_make("queue",        "queue");
    sink        = gst_element_factory_make("filesink",     "file-output");

    if (!pipeline || !source || !framefilter || !parse || !qtic2venc || !muxer || !queue || !sink) {
    ...
    gst_bin_add_many(GST_BIN(pipeline), source, framefilter, qtic2venc, parse, muxer, queue, sink, NULL);
    gst_element_link_many(source, framefilter, qtic2venc, parse, muxer, queue, sink, NULL);
