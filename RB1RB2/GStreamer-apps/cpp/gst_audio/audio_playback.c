...
static int playback(const char *format, const char *content)
{
    ...
    GstElement *wavparse;
    GstElement *aacparse, *avdec_aac, *audioconvert;
    ...
    if (!strncmp(format, "wav", 8)) {
        ...
        gst_bin_add_many(GST_BIN(pipeline), filesrc, aacparse, avdec_aac, audioconvert, pulsesink, NULL);
        if (gst_element_link_many(filesrc, aacparse, avdec_aac, audioconvert, pulsesink, NULL) != TRUE) {
            g_printerr("Linked elements failed.\n");
            gst_object_unref(pipeline);
            return GST_STATE_NULL;
        }
        ...
        gst_bin_add_many(GST_BIN(pipeline), filesrc, mpegaudioparse, avdec_mp3, audioconvert, pulsesink, NULL);
        if (gst_element_link_many(filesrc, mpegaudioparse, avdec_mp3, audioconvert, pulsesink, NULL) != TRUE) {
            g_printerr("Linked elements failed.\n");
            gst_object_unref(pipeline);
            return GST_STATE_NULL;
        }
