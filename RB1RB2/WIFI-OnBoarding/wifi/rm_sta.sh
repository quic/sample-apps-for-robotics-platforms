...
if [ -z "$judge_conf" ]; then
    ...
    wpa_supplicant -i$network_card -Dnl80211 \
        -c /etc/wlan/wpa_supplicant.conf \
        -O /etc/wlan/sockets -B
else
...
if [ "$judge_result" = "" ]; then
    ...
    # Delete saved WiFi #
    ret_rm=$(wpa_cli -i $network_card remove_network $num -p /etc/wlan/sockets)
    echo -e "\e[32mRemove #$num WIFI $ret_rm\e[0m"

    # Update configuration table #
    ret_save=$(wpa_cli -i $network_card save_config -p /etc/wlan/sockets)
